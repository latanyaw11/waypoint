...
router.get('/', async (req, res, next) => {
  try {
    let query = supabase.from('celebrity_guides').select('*').eq('is_published', true).order('sort_weight', { ascending: false });
...
router.patch('/:id', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    const { data, error } = await supabase.from('celebrity_guides').update(req.body).eq('id', req.params.id).select().single();
    if (error) throw error;
    res.json(data);
  } catch (e) { next(e); }
});
...
router.delete('/:id', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    const { error } = await supabase.from('celebrity_guides').delete().eq('id', req.params.id);
    if (error) throw error;
    res.status(204).send();
  } catch (e) { next(e); }
});
