...
// POST /api/trips/:tripId/places/reorder — save manual drag-and-drop order
router.post('/reorder', requireAuth, async (req, res, next) => {
  try {
    const { order } = req.body; // [{ id, position }]
    if (!Array.isArray(order)) return res.status(400).json({ error: 'order must be an array' });
    // Update each place's route_position
    await Promise.all(order.map(({ id, position }) =>
      supabase.from('places').update({ route_position: position }).eq('id', id).eq('trip_id', req.params.tripId)
    ));
    res.json({ ok: true });
  } catch (e) { next(e); }
});
